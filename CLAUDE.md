...
- **キープアライブ（`GET /healthz`）＝Renderの起動画面対策**：Render無料プランは[15分アクセスが無いと停止し、復帰に約1分](https://render.com/docs/free)。その間**Render側のローディング画面**が出て、患者さんが「知らないサイトが出た＝ウイルス？」と不審に思う。**この画面はカスタマイズ不可**（アプリ起動前にRenderのedgeが出すため）。対策として外部cronから起きている時間帯（7〜23時JST）だけ10分おきに`/healthz`を叩いて起こしておく。`/healthz`はRedisにも外部にも触らず200を返すだけ。
  - **服薬リマインダーのcronを流用しないこと**：あちらは`currentHHMM < reminder.time`でスキップする判定のため、**cronの時間帯を絞ると、絞った終了時刻より後ろに設定されたリマインダーが永久に発火しない**（翌朝も`07:00 < 23:30`で再スキップされるため）。リマインダーのcronは24時間動かしたまま、キープアライブは別のcronで行う
  - **無料枠は750インスタンス時間/月・ワークスペース単位**。**使い切ると翌月まで無料Webサービスが全停止**するので、24時間キープアライブ（31日＝744時間で残り6時間）は医療サービスとして危険。7〜23時なら約500時間で余裕がある。恒久対策はStarterプラン（$7/月・512MB/0.5CPU）でスピンダウン自体を無くすこと
  - **未検証**：スピンダウン中にLINEのwebhookが来ると復帰の約1分待たされ、**応答トークンの期限切れで患者さんに返信が届かない**おそれがある。LINE公式ドキュメントからトークンの有効期限を確認できていない。15分以上放置してからLINEに送って返信が来るかで判定できる
