...
- **新薬対応＝PMDA限定Web検索＋モデル更新（`claudeHandler.js`）**：「比較的新しい薬（新薬）について答えられない」への対処。モデルの知識にはカットオフがあるため、**自前で添付文書を持つのではなく、質問時にAnthropicのWeb検索サーバーツールで公式情報を読ませる**方式。LINE・HPとも`askClaude`を通るので1か所で効く。
  ...
  - **`content[0].text`は回答とは限らない**：検索が走ると`content`に`server_tool_use`・検索結果・内部のコード実行結果が混ざる。`collectAnswer()`が**最後のツールブロックより後ろの`text`だけ**を集める（前方のテキストは「検索しますね」という実況で、患者さんに出してはいけない）。検索が走らなければツールブロックが無く全`text`が回答。回答が取り出せない異常時は**空メッセージを送らず必ずエスカレーション**する
  - **`pause_turn`のハンドリング必須**：検索が長引くとAPIがturnを中断する。中断応答をそのまま戻して継続（`tools`を外すと未実行の検索が残っていてエラー）。`MAX_PAUSE_CONTINUATIONS`で上限＝青天井の課金を防ぐ
  - **引用元URLを併記**（`sources`）。APIの規約上、検索結果を根拠にした回答を利用者に見せる場合に必要。患者さんが一次情報を確認できる利点も
  - **検索の失敗はHTTP 200で返る**（`web_search_tool_result_error`）。例外にならないので`logSearchUsage`でログに残す。`usage.server_tool_use.web_search_requests`で実際の検索回数＝課金も追える
  - **費用**：検索$10/1,000回＋検索結果の入力トークン。**Claudeは必要な時しか検索しない**（「飲み忘れたら？」等の安定知識では走らない）＝コストが増える通と今答えられていない通がほぼ一致する。`max_uses:3`で上限固定
  - **モデルは2系統に分けている**：チャットは`MODEL='claude-sonnet-5'`、**写真読み取りは`IMAGE_MODEL='claude-sonnet-4-6'`で据え置き**。写真経路はベンチマークで詰めた結果（下記）であり、再ベンチマークなしにモデルを変えないこと。ZDR非対応（`web_search_20260209`は内部でコード実行を使うため）
